...
def main():
    ...
    img_list = sorted(glob.glob(c.IMAGE_PATH_cover + '/*')) 
    #img_list = sorted(glob.glob(c.IMAGE_PATH_secret + '/*'))
    ...
    for i, img_path in enumerate(img_list):
        ...
        #im_GT = cv2.imread(img_path).astype(np.float32) / 255.0
        # print(base_name)
        # print(img_path)
        # print(os.path.join(folder_Gen, base_name + '.png'))
        
        im_Gen = cv2.imread(os.path.join(c.IMAGE_PATH_steg, base_name + '.png')) / 255.
        #im_Gen = cv2.imread(os.path.join(c.IMAGE_PATH_secret_rev, base_name + '.png')) / 255.
        ...
        RMSE = calculate_rmse(im_GT * 255, im_Gen * 255)
        MAE = calculate_mae(im_GT * 255, im_Gen * 255)
        
        
        
        # Print PSNR, SSIM, RMSE, and MAE
        print('{:3d} - {:25}. \tPSNR: {:.6f} dB, \tSSIM: {:.6f}, \tRMSE: {:.6f}, \tMAE: {:.6f}'.format(
            i + 1, base_name, PSNR ,SSIM, RMSE, MAE))
# =============================================================================
#         if PSNR < 28:
#             # Remove the images from both folders
#             try:
#                 os.remove(img_path)
#                 os.remove(os.path.join(c.IMAGE_PATH_secret_rev, base_name + '.jpg'))
#                 print(f"Images {base_name} removed due to PSNR < 20.")
#                 counter = counter + 1 ;
#             except Exception as e:
#                 print(f"Error removing images {base_name}: {str(e)}")
# =============================================================================
